...
import { CODESIGN_TIMEOUT_MS, DOWNLOAD_TIMEOUT_MS, LIST_TIMEOUT_MS, MAX_BINARY_BYTES, MAX_CHECKSUM_BYTES, } from "./constants.js";
...
function trustedDownloadUrl(asset, target, releasesApi) {
    ...
    const trusted = releasesApi === defaultReleasesApi(target)
...
export async function downloadAsset(release, destination, query) {
    const { asset } = release;
...
export const verifyAdHocSignature = (binary) => new Promise((resolve, reject) => {
    execFile("/usr/bin/codesign", ["--verify", "--strict", binary], { timeout: CODESIGN_TIMEOUT_MS }, (error) => (error ? reject(error) : resolve()));
});
