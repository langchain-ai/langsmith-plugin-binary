...
export function buildPlan({ config, repositoryRoot }: LoadedConfig): BuildPlan {
  ...
  const defines: Record<string, string> = { [VERSION_DEFINE]: version };
...
export function requestedArches(plan: BuildPlan, argv: string[]): string[] {
  const requested = argv.find((arg) => arg.startsWith("--arch="))?.slice("--arch=".length);
  if (requested === "all") return [...plan.arches];
  const arch = requested ?? process.arch;
  if (!plan.arches.includes(arch)) {
    throw new Error(
      `Unsupported architecture ${arch}, expected one of ${[...plan.arches, "all"].join(", ")}`,
    );
  }
  return [arch];
}

export function outputPath(plan: BuildPlan, arch: string): string {
  if (arch === process.arch) return join(plan.outputDirectory, plan.executableName);
  return join(plan.outputDirectory, `${plan.platform}-${arch}`, plan.executableName);
}

export function buildArguments(plan: BuildPlan, arch: string, binary: string): string[] {
  return [
    "build",
    "--compile",
...
export function build(
  ...
  for (const arch of requestedArches(plan, argv)) {
    ...
    if (arch === process.arch) steps.checkVersion(binary, plan.version);
    log(
      `Built the ${plan.platform}-${arch} binary ${binary} (${
        statSync(binary).size
      } bytes, version ${plan.version})`,
    );
  }
}
