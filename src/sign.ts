...
import type { AppleCredentials, Environment, LoadedConfig } from "./models.js";
import {
...
export async function sign(
  ...
  if (!existsSync(target)) throw new Error(`there is no binary to sign at ${target}`);
  const entitlements = resolve(loaded.repositoryRoot, loaded.config.sign.entitlements);
  const { identity, submissionId } = await signAndNotarize(
    target,
    entitlements,
    env as AppleCredentials,
  );
