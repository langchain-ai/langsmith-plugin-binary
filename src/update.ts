...
function requirePublishedTarget(target: BinaryTarget, platform: string, arch: string): void {
...
export async function installFromReleases(
  target: BinaryTarget,
...
export async function installLocalCopy(
  target: BinaryTarget,
  ...
  const installDir = resolveInstallDir(target, options);
  const path = await installRunningBinary(
    target,
    executablePath,
    installDir,
    version,
