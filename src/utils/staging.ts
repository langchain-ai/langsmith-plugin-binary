...
function keptLine(staged: string | undefined): string {
  return staged
    ? `\nThe copy it tried to run is kept at ${staged} so you can run it yourself.`
    : "";
}

export function worthRetrying(failure: VersionCheckFailure): boolean {
  return failure.kind === "stopped";
}

export function worthKeeping(failure: VersionCheckFailure): boolean {
  return failure.kind !== "exit";
}

export function whyTheVersionIsWrong(reported: string, expected: string): string {
  return `The downloaded binary reports version ${reported}, expected ${expected}.`;
}

export function whyTheSignatureStoppedMatching(error: unknown, staged: string | undefined): string {
...
export function whyTheVersionCheckFailed(
